...
namespace Spartane.Web.Areas.Frontal.Controllers
{
    ...
    public class Spartan_Traduction_ProcessController : Controller
    {
        ...
        private ISpartanObjectApiConsumer _ISpartanObjectApiConsumer;
        ...
        public ActionResult Index(int ModuleId=0)
        {
			/*if (ModuleId == 0)
            {
                ...
                if (ModuleId == 0)
                {
                    Response.Redirect("~/Frontal/Accounts/UnAuthorized?controllerName=Spartan_Traduction_Process&ActionName=Index");
                }
            }*/
        ...
        public ActionResult Create(int Id = 0,  int consult = 0, int ModuleId=0)
        {
           ...
           if ((!permission.New && Id.ToString() == "0") || (!permission.Edit && Id.ToString() != "0" && (!permission.Consult && consult == 1)))
            {
                Response.Redirect("~/Frontal/Accounts/UnAuthorized?controllerName=Spartan_Traduction_Process&ActionName=Create");
            }
            ViewBag.Permission = permission;
	...
	[HttpGet]
        public ActionResult AddSpartan_Traduction_Process(int rowIndex = 0, int functionMode = 0, string id = "0")
        {
           ...
           _ISpartan_Traduction_ProcessApiConsumer.SetAuthHeader(_tokenManager.Token);
            ...
            var permissionSpartan_Traduction_Detail = PermissionHelper.GetRoleObjectPermission(SessionHelper.Role, 32363, ModuleId);
            ViewBag.PermissionSpartan_Traduction_Detail = permissionSpartan_Traduction_Detail;
   ...
   _ISpartanObjectApiConsumer.SetAuthHeader(_tokenManager.Token);
        ...
        [HttpPost]
        public ActionResult ShowAdvanceFilter(Spartan_Traduction_ProcessAdvanceSearchModel model)
        {
   ...
   _ISpartanObjectApiConsumer.SetAuthHeader(_tokenManager.Token);
        ...
        [HttpGet]
        public JsonResult GetSpartan_Traduction_Process_ObjectT_SpartanObject(string query, string where)
        {
            try
            {
                if (String.IsNullOrEmpty(where))
                ...
                var result = _ISpartanObjectApiConsumer.ListaSelAll(1, 20, " (cast(SpartanObject.Object_Id as nvarchar(max)) LIKE '%" + query.Trim() + "%' or cast(SpartanObject.Name as nvarchar(max)) LIKE '%" + query.Trim() + "%') " + where, " SpartanObject.Name ASC ").Resource;
                return Json(result.SpartanObjects.ToArray(), JsonRequestBehavior.AllowGet);
            }
            catch (ServiceException ex)
            {
                return Json(null, JsonRequestBehavior.AllowGet);
...
//Grid GetAutoComplete
        ...
        public ActionResult Delete(int id)
        {
            try
            {
                ...
                Spartan_Traduction_Process varSpartan_Traduction_Process = null;
                if (id.ToString() != "0")
                {
                        string where = "";
                    _ISpartan_Traduction_DetailApiConsumer.SetAuthHeader(_tokenManager.Token);
                    where = "Spartan_Traduction_Detail.Spartan_Traduction_Process=" + id;
                    if("int" == "string")
                    {
	                where = "Spartan_Traduction_Detail.Spartan_Traduction_Process='" + id + "'";
                    }
                    var Spartan_Traduction_DetailInfo =
                        _ISpartan_Traduction_DetailApiConsumer.ListaSelAll(1, int.MaxValue, where,"").Resource;

                    if (Spartan_Traduction_DetailInfo.Spartan_Traduction_Details.Count > 0)
                    {
                        var resultSpartan_Traduction_Detail = true;
                        //Removing associated job history with attachment
                        foreach (var Spartan_Traduction_DetailItem in Spartan_Traduction_DetailInfo.Spartan_Traduction_Details)
                            resultSpartan_Traduction_Detail = resultSpartan_Traduction_Detail
                                              && _ISpartan_Traduction_DetailApiConsumer.Delete(Spartan_Traduction_DetailItem.IdTraductionDetail, null,null).Resource;

                        if (!resultSpartan_Traduction_Detail)
                            return Json(false, JsonRequestBehavior.AllowGet);
                    }

                }
                var result = _ISpartan_Traduction_ProcessApiConsumer.Delete(id, null, null).Resource;
        ...
        {
            CultureHelper.ProcessResources();
