...
function ClearFormControls() {
    $('#CreateCiudad').trigger('reset');
    $('#CreateCiudad').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
$(document).ready(function () {
    $("form#CreateCiudad").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateCiudad").on('click', '#CiudadCancelar', function () {
        BackToGrid();
    });
	$("form#CreateCiudad").on('click', '#CiudadGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendCiudadData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateCiudad").on('click', '#CiudadGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendCiudadData(function () {
                ClearControls();
    ...
    $("form#CreateCiudad").on('click', '#CiudadGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendCiudadData(function (currentId) {
                $("#ClaveId").val("0");
...
var CiudadisdisplayBusinessPropery = false;
CiudadDisplayBusinessRuleButtons(CiudadisdisplayBusinessPropery);
function CiudadDisplayBusinessRule() {
    if (!CiudadisdisplayBusinessPropery) {
        $('#CreateCiudad').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="displayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#CiudadPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.CiudaddisplayBusinessPropery').remove();
    }
    CiudadDisplayBusinessRuleButtons(!CiudadisdisplayBusinessPropery);
    CiudadisdisplayBusinessPropery = (CiudadisdisplayBusinessPropery ? false : true);
}
function CiudadDisplayBusinessRuleButtons(flag) {
