...
function ClearControls() {
    ...
    $('#CreateDetalle_Familiar_de_Vendedor')[0].reset();
    ClearFormControls();
    $("#FolioId").val("0");
...
function ClearFormControls() {
    $('#CreateDetalle_Familiar_de_Vendedor').trigger('reset');
    $('#CreateDetalle_Familiar_de_Vendedor').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateDetalle_Familiar_de_Vendedor');
...
$(document).ready(function () {
    $("form#CreateDetalle_Familiar_de_Vendedor").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateDetalle_Familiar_de_Vendedor").on('click', '#Detalle_Familiar_de_VendedorCancelar', function () {
        BackToGrid();
    });
	$("form#CreateDetalle_Familiar_de_Vendedor").on('click', '#Detalle_Familiar_de_VendedorGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendDetalle_Familiar_de_VendedorData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateDetalle_Familiar_de_Vendedor").on('click', '#Detalle_Familiar_de_VendedorGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendDetalle_Familiar_de_VendedorData(function () {
                ClearControls();
    ...
    $("form#CreateDetalle_Familiar_de_Vendedor").on('click', '#Detalle_Familiar_de_VendedorGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendDetalle_Familiar_de_VendedorData(function (currentId) {
                $("#FolioId").val("0");
...
var Detalle_Familiar_de_VendedorisdisplayBusinessPropery = false;
Detalle_Familiar_de_VendedorDisplayBusinessRuleButtons(Detalle_Familiar_de_VendedorisdisplayBusinessPropery);
function Detalle_Familiar_de_VendedorDisplayBusinessRule() {
    if (!Detalle_Familiar_de_VendedorisdisplayBusinessPropery) {
        $('#CreateDetalle_Familiar_de_Vendedor').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="displayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Detalle_Familiar_de_VendedorPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Detalle_Familiar_de_VendedordisplayBusinessPropery').remove();
    }
    Detalle_Familiar_de_VendedorDisplayBusinessRuleButtons(!Detalle_Familiar_de_VendedorisdisplayBusinessPropery);
    Detalle_Familiar_de_VendedorisdisplayBusinessPropery = (Detalle_Familiar_de_VendedorisdisplayBusinessPropery ? false : true);
}
function Detalle_Familiar_de_VendedorDisplayBusinessRuleButtons(flag) {
