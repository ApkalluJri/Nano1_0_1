...
function ClearControls() {
    ...
    $('#CreateDetalle_Sucursal_de_Empresa')[0].reset();
    ClearFormControls();
    $("#FolioId").val("0");
...
function ClearFormControls() {
    $('#CreateDetalle_Sucursal_de_Empresa').trigger('reset');
    $('#CreateDetalle_Sucursal_de_Empresa').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateDetalle_Sucursal_de_Empresa');
...
$(document).ready(function () {
    $("form#CreateDetalle_Sucursal_de_Empresa").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateDetalle_Sucursal_de_Empresa").on('click', '#Detalle_Sucursal_de_EmpresaCancelar', function () {
        BackToGrid();
    });
	$("form#CreateDetalle_Sucursal_de_Empresa").on('click', '#Detalle_Sucursal_de_EmpresaGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendDetalle_Sucursal_de_EmpresaData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateDetalle_Sucursal_de_Empresa").on('click', '#Detalle_Sucursal_de_EmpresaGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendDetalle_Sucursal_de_EmpresaData(function () {
                ClearControls();
    ...
    $("form#CreateDetalle_Sucursal_de_Empresa").on('click', '#Detalle_Sucursal_de_EmpresaGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendDetalle_Sucursal_de_EmpresaData(function (currentId) {
                $("#FolioId").val("0");
...
var Detalle_Sucursal_de_EmpresaisdisplayBusinessPropery = false;
Detalle_Sucursal_de_EmpresaDisplayBusinessRuleButtons(Detalle_Sucursal_de_EmpresaisdisplayBusinessPropery);
function Detalle_Sucursal_de_EmpresaDisplayBusinessRule() {
    if (!Detalle_Sucursal_de_EmpresaisdisplayBusinessPropery) {
        $('#CreateDetalle_Sucursal_de_Empresa').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="displayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Detalle_Sucursal_de_EmpresaPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Detalle_Sucursal_de_EmpresadisplayBusinessPropery').remove();
    }
    Detalle_Sucursal_de_EmpresaDisplayBusinessRuleButtons(!Detalle_Sucursal_de_EmpresaisdisplayBusinessPropery);
    Detalle_Sucursal_de_EmpresaisdisplayBusinessPropery = (Detalle_Sucursal_de_EmpresaisdisplayBusinessPropery ? false : true);
}
function Detalle_Sucursal_de_EmpresaDisplayBusinessRuleButtons(flag) {
