...
namespace MS_Observatorio_por_rolCS
{
    public class MS_Observatorio_por_rolFilters
    {
...
//        private Registro_de_RolesCS.Registro_de_RolesFilters[] varClave;
...
public class objectBussinessMS_Observatorio_por_rol : IDisposable
{
    ...
    private MS_Observatorio_por_rolFilters[] filters; 
    ...
    public MS_Observatorio_por_rolFilters[] Filters
    {
        get { return filters; }
        set { filters = value; }
    }
    public MS_Observatorio_por_rolFilters Filter
    {
        ///Cuando es llamada desde las relaciones de algun catalago
        set
        {
            filters = new MS_Observatorio_por_rolFilters[1];
            filters[0] = value;
        }
    }
    ...
    public Int32 SelCount()
    {
        ...
        if (swhere != "")
        {
            ...
            com.CommandText = "sp_SelAllComplete_MS_Observatorio_por_rol_WithFilter";
        }else 
            com.CommandText = "sp_SelAllComplete_MS_Observatorio_por_rol";
            ...
            if (this.filters !=null)
                foreach (MS_Observatorio_por_rolCS.MS_Observatorio_por_rolFilters fil in this.filters)
                {
                    String sWhereT = "";
                    sWhereT = Function.CreateQuerySelect("MS_Observatorio_por_rol", fil);
                    if (sWhereT != "")
    ...
    public DataSet SelAll(Boolean ConRelaciones)
    {
        ...
        if (ConRelaciones ==true)
            ...
            com.CommandText="sp_SelAllMS_Observatorio_por_rol";
            ds = db.Consulta(com);
            ...
            if (this.filters !=null)
                foreach (MS_Observatorio_por_rolCS.MS_Observatorio_por_rolFilters fil in this.filters)
                {
                    ...
                    if (sWhereT != "")
                        swhere = swhere + " And "+sWhereT;
                    String sOrderByT = "";
                    sOrderByT = Function.CreateOrderBySelect("MS_Observatorio_por_rol", fil);
                    if (sOrderByT != "")
                        sOrderBy = sOrderByT;
                }
        if (swhere != "")
    ...
    public int SelCount(string sortDirection, string sortExpression, int maximumRows, int startRowIndex)
    {
        ...
        string sPagingCountTemplate = @"SELECT COUNT(*) {0}";
                  string from = " from (( MS_Observatorio_por_rol WITH(NoLock) LEFT JOIN Registro_de_Roles as Clave WITH(NoLock) ON Clave.Folio=MS_Observatorio_por_rol.Clave)       LEFT JOIN Observatorio as Observatorio WITH(NoLock) ON Observatorio.Clave=MS_Observatorio_por_rol.Observatorio)        " + swhere; 
    ...
    public DataSet SelAll(string sortDirection, string sortExpression, int maximumRows, int startRowIndex)
    {
        ...
        if (swhere != string.Empty)
            swhere = (!swhere.ToLower().Trim().StartsWith("where") ? " WHERE " + swhere : swhere);
                    ...
                    switch (sortExpression)             {                 case "MS_Observatorio_por_rol_Clave": sortExpression = "MS_Observatorio_por_rol.Clave"; break;  case "Clave_Folio": sortExpression = "Clave.Folio"; break;  case "MS_Observatorio_por_rol_Observatorio": sortExpression = "MS_Observatorio_por_rol.Observatorio"; break;  case "Observatorio_Nombre": sortExpression = "Observatorio.Nombre"; break;                               }              sortExpression = (sortExpression == string.Empty || sortExpression == null ? "MS_Observatorio_por_rol.Clave, Clave.Folio, MS_Observatorio_por_rol.Observatorio, Observatorio.Nombre" : sortExpression);             if (sortDirection != string.Empty)                 sortDirection = (sortDirection.ToLower() == "ascending" ? "Asc" : "Desc");             string fieldsWithAlias = " MS_Observatorio_por_rol.Clave AS MS_Observatorio_por_rol_Clave,        Clave.Folio AS Clave_Folio,        MS_Observatorio_por_rol.Observatorio AS MS_Observatorio_por_rol_Observatorio,        Observatorio.Nombre AS Observatorio_Nombre ";             string fieldsForExport = " MS_Observatorio_por_rol.Clave AS [Clave],        Clave.Folio AS [Clave Folio],        MS_Observatorio_por_rol.Observatorio AS [Observatorio],        Observatorio.Nombre AS [Observatorio Nombre] ";             string from = " from (( MS_Observatorio_por_rol WITH(NoLock) LEFT JOIN Registro_de_Roles as Clave WITH(NoLock) ON Clave.Folio=MS_Observatorio_por_rol.Clave)       LEFT JOIN Observatorio as Observatorio WITH(NoLock) ON Observatorio.Clave=MS_Observatorio_por_rol.Observatorio)        " + swhere; 
    ...
    {
        	swhere = where; 	string fieldsWithAlias = " MS_Observatorio_por_rol.Clave AS MS_Observatorio_por_rol_Clave,        Clave.Folio AS Clave_Folio,        MS_Observatorio_por_rol.Observatorio AS MS_Observatorio_por_rol_Observatorio,        Observatorio.Nombre AS Observatorio_Nombre ";         Order = (Order == string.Empty || Order == null ? "MS_Observatorio_por_rol.Clave, Clave.Folio, MS_Observatorio_por_rol.Observatorio, Observatorio.Nombre" : Order);         string from = " from (( MS_Observatorio_por_rol WITH(NoLock) LEFT JOIN Registro_de_Roles as Clave WITH(NoLock) ON Clave.Folio=MS_Observatorio_por_rol.Clave)       LEFT JOIN Observatorio as Observatorio WITH(NoLock) ON Observatorio.Clave=MS_Observatorio_por_rol.Observatorio)        " + swhere;
    ...
    public void Insert(TTSecurity.GlobalData UserInformation, TTDataLayerCS.DataLayerFieldsBitacora DataReference){
        ...
        db.BeginTransaction("MS_Observatorio_por_rol");
        int? sKeyClave = -1;
        try
        {
        ...
        catch (Exception ex)
        {
            try
            {
                db.RollBackTransaction("MS_Observatorio_por_rol");
            }
            catch{}
            throw ex; 
        }
    ...
    public void Update(TTSecurity.GlobalData UserInformation, TTDataLayerCS.DataLayerFieldsBitacora DataReference){
        ...
        try
        {
            ...
            SqlCommand com = new SqlCommand();
            com.CommandText="sp_UpdMS_Observatorio_por_rol";
            com.Parameters.AddWithValue("@Clave",Conversion.FormatNull(varClave));
            com.Parameters.AddWithValue("@Observatorio",Conversion.FormatNull(varObservatorio));
...
//            varClave = sKeyClave;
    ...
    public bool Delete(int? KeyClave, int? KeyObservatorio, TTSecurity.GlobalData UserInformation, TTDataLayerCS.DataLayerFieldsBitacora DataReference){
            ...
            try
            {
                ...
                SqlCommand com = new SqlCommand();
                com.CommandText="sp_DelMS_Observatorio_por_rol";
com.Parameters.AddWithValue("@Clave",KeyClave);com.Parameters.AddWithValue("@Observatorio",KeyObservatorio);
    ...
    private DataSet GetByKey(DataSet ds)
    {
            ...
            {
            Clave = Conversion.CambiaToShort(ds.Tables[0].Rows[0]["MS_Observatorio_por_rol_Clave"]);
            Observatorio = Conversion.CambiaToShort(ds.Tables[0].Rows[0]["MS_Observatorio_por_rol_Observatorio"]);
...
//        {
            ...
            SqlCommand com = new SqlCommand();
            if (ConRelaciones==true)
                com.CommandText="sp_GetComplete_MS_Observatorio_por_rol";
            else
                com.CommandText="sp_GetMS_Observatorio_por_rol";
com.Parameters.AddWithValue("@Clave",KeyClave);com.Parameters.AddWithValue("@Observatorio",KeyObservatorio);
...
//        {
            TTDataLayerCS.BD db =new TTDataLayerCS.BD();
            SqlCommand com = new SqlCommand();
            com.CommandText="Select         MS_Observatorio_por_rol.Clave AS MS_Observatorio_por_rol_Clave,        Registro_de_Roles.Folio AS Registro_de_Roles_Folio,        MS_Observatorio_por_rol.Observatorio AS MS_Observatorio_por_rol_Observatorio,        Observatorio.Nombre AS Observatorio_Nombre   from (( MS_Observatorio_por_rol       LEFT JOIN Registro_de_Roles ON Registro_de_Roles.Folio=MS_Observatorio_por_rol.Clave)       LEFT JOIN Observatorio ON Observatorio.Clave=MS_Observatorio_por_rol.Observatorio)           Where MS_Observatorio_por_rol.Clave = '" + Key + "'";
            com.CommandType=CommandType.Text;
            ds = db.Consulta(com);
            return GetByKey(ds);
        //}
...
//        {
            TTDataLayerCS.BD db =new TTDataLayerCS.BD();
            SqlCommand com = new SqlCommand();
            com.CommandText="Select         MS_Observatorio_por_rol.Clave AS MS_Observatorio_por_rol_Clave,        Registro_de_Roles.Folio AS Registro_de_Roles_Folio,        MS_Observatorio_por_rol.Observatorio AS MS_Observatorio_por_rol_Observatorio,        Observatorio.Nombre AS Observatorio_Nombre   from (( MS_Observatorio_por_rol       LEFT JOIN Registro_de_Roles ON Registro_de_Roles.Folio=MS_Observatorio_por_rol.Clave)       LEFT JOIN Observatorio ON Observatorio.Clave=MS_Observatorio_por_rol.Observatorio)           Where MS_Observatorio_por_rol.Observatorio = '" + Key + "'";
            com.CommandType=CommandType.Text;
            ds = db.Consulta(com);
            return GetByKey(ds);
        //}
    ...
    public Int32 CurrentPosicion(int? KeyClave, int? KeyObservatorio)
    {
        ...
        {
            if (Function.FormatNumberNull(dt.Rows[i]["MS_Observatorio_por_rol_Clave"]) == KeyClave&& Function.FormatNumberNull(dt.Rows[i]["MS_Observatorio_por_rol_Observatorio"]) == KeyObservatorio)
                return i;
        }
        return -1;
    }
...
*///            public DataTable FillDataClave(Object ctr, String Filtro){
            public DataTable FillDataClave(String Filtro){
                TTDataLayerCS.BD db = new TTDataLayerCS.BD();
                SqlCommand com = new SqlCommand();
                com.CommandText = "sp_selMS_Observatorio_por_rol_Relacion_Registro_de_Roles";
                com.CommandType = CommandType.StoredProcedure;
                //ctr.Items.Clear();
                DataTable dt = db.Consulta(com).Tables[0];
