...
namespace Rol_de_UsuarioCS
{
    public class Rol_de_UsuarioFilters
    {
...
public class objectBussinessRol_de_Usuario : IDisposable
{
    ...
    private Rol_de_UsuarioFilters[] filters; 
    ...
    public Rol_de_UsuarioFilters[] Filters
    {
        get { return filters; }
        set { filters = value; }
    }
    public Rol_de_UsuarioFilters Filter
    {
        ///Cuando es llamada desde las relaciones de algun catalago
        set
        {
            filters = new Rol_de_UsuarioFilters[1];
            filters[0] = value;
        }
    }
    ...
    public Int32 SelCount()
    {
        ...
        if (swhere != "")
        {
            ...
            com.CommandText = "sp_SelAllComplete_Rol_de_Usuario_WithFilter";
        }else 
            com.CommandText = "sp_SelAllComplete_Rol_de_Usuario";
            ...
            if (this.filters !=null)
                foreach (Rol_de_UsuarioCS.Rol_de_UsuarioFilters fil in this.filters)
                {
                    String sWhereT = "";
                    sWhereT = Function.CreateQuerySelect("Rol_de_Usuario", fil);
                    if (sWhereT != "")
    ...
    public DataSet SelAll(Boolean ConRelaciones)
    {
        ...
        if (ConRelaciones ==true)
            ...
            com.CommandText="sp_SelAllRol_de_Usuario";
            ds = db.Consulta(com);
            ...
            if (this.filters !=null)
                foreach (Rol_de_UsuarioCS.Rol_de_UsuarioFilters fil in this.filters)
                {
                    ...
                    if (sWhereT != "")
                        swhere = swhere + " And "+sWhereT;
                    String sOrderByT = "";
                    sOrderByT = Function.CreateOrderBySelect("Rol_de_Usuario", fil);
                    if (sOrderByT != "")
                        sOrderBy = sOrderByT;
                }
        if (swhere != "")
    ...
    public int SelCount(string sortDirection, string sortExpression, int maximumRows, int startRowIndex)
    {
        ...
        string sPagingCountTemplate = @"SELECT COUNT(*) {0}";
                  string from = " from  Rol_de_Usuario WITH(NoLock)  " + swhere; 
    ...
    public DataSet SelAll(string sortDirection, string sortExpression, int maximumRows, int startRowIndex)
    {
        ...
        if (swhere != string.Empty)
            swhere = (!swhere.ToLower().Trim().StartsWith("where") ? " WHERE " + swhere : swhere);
                    ...
                    switch (sortExpression)             {                 case "Rol_de_Usuario_Clave": sortExpression = "Rol_de_Usuario.Clave"; break;  case "Rol_de_Usuario_Descripcion": sortExpression = "Rol_de_Usuario.Descripcion"; break;                               }              sortExpression = (sortExpression == string.Empty || sortExpression == null ? "Rol_de_Usuario.Clave, Rol_de_Usuario.Descripcion" : sortExpression);             if (sortDirection != string.Empty)                 sortDirection = (sortDirection.ToLower() == "ascending" ? "Asc" : "Desc");             string fieldsWithAlias = " Rol_de_Usuario.Clave As Rol_de_Usuario_Clave,        Rol_de_Usuario.Descripcion As Rol_de_Usuario_Descripcion ";             string fieldsForExport = " Rol_de_Usuario.Clave As [Clave],        Rol_de_Usuario.Descripcion As [Descripcion] ";             string from = " from  Rol_de_Usuario WITH(NoLock)  " + swhere; 
    ...
    {
        	swhere = where; 	string fieldsWithAlias = " Rol_de_Usuario.Clave As Rol_de_Usuario_Clave,        Rol_de_Usuario.Descripcion As Rol_de_Usuario_Descripcion ";         Order = (Order == string.Empty || Order == null ? "Rol_de_Usuario.Clave, Rol_de_Usuario.Descripcion" : Order);         string from = " from  Rol_de_Usuario WITH(NoLock)  " + swhere;
    ...
    public void Insert(TTSecurity.GlobalData UserInformation, TTDataLayerCS.DataLayerFieldsBitacora DataReference){
        ...
        db.BeginTransaction("Rol_de_Usuario");
        int? sKeyClave = -1;
        try
        {

            SqlCommand com = new SqlCommand();
            com.CommandText="sp_InsRol_de_Usuario";
        ...
        catch (Exception ex)
        {
            try
            {
                db.RollBackTransaction("Rol_de_Usuario");
            }
            catch{}
            throw ex; 
        }
    ...
    public void Update(TTSecurity.GlobalData UserInformation, TTDataLayerCS.DataLayerFieldsBitacora DataReference){
        ...
        try
        {
            ...
            SqlCommand com = new SqlCommand();
            com.CommandText="sp_UpdRol_de_Usuario";
            com.Parameters.AddWithValue("@Clave",Conversion.FormatNull(varClave));
            if (varDescripcion!=null)
            {
                if (varDescripcion != "")
...
//            varClave = sKeyClave;
    ...
    public bool Delete(int? KeyClave, TTSecurity.GlobalData UserInformation, TTDataLayerCS.DataLayerFieldsBitacora DataReference){
            ...
            try
            {
                ...
                SqlCommand com = new SqlCommand();
                com.CommandText="sp_DelRol_de_Usuario";
com.Parameters.AddWithValue("@Clave",KeyClave);
    ...
    private DataSet GetByKey(DataSet ds)
    {
            ...
            {
            Clave = Conversion.CambiaToShort(ds.Tables[0].Rows[0]["Rol_de_Usuario_Clave"]);
            Descripcion = ds.Tables[0].Rows[0]["Rol_de_Usuario_Descripcion"].ToString();
...
//        {
            ...
            SqlCommand com = new SqlCommand();
            if (ConRelaciones==true)
                com.CommandText="sp_GetComplete_Rol_de_Usuario";
            else
                com.CommandText="sp_GetRol_de_Usuario";
com.Parameters.AddWithValue("@Clave",KeyClave);
...
//        {
            TTDataLayerCS.BD db =new TTDataLayerCS.BD();
            SqlCommand com = new SqlCommand();
            com.CommandText="Select         Rol_de_Usuario.Clave As Rol_de_Usuario_Clave,        Rol_de_Usuario.Descripcion As Rol_de_Usuario_Descripcion   from  Rol_de_Usuario           Where Rol_de_Usuario.Clave = '" + Key + "'";
            com.CommandType=CommandType.Text;
            ds = db.Consulta(com);
            return GetByKey(ds);
        //}
...
//        {
            TTDataLayerCS.BD db =new TTDataLayerCS.BD();
            SqlCommand com = new SqlCommand();
            com.CommandText="Select         Rol_de_Usuario.Clave As Rol_de_Usuario_Clave,        Rol_de_Usuario.Descripcion As Rol_de_Usuario_Descripcion   from  Rol_de_Usuario           Where Rol_de_Usuario.Descripcion = '" + Key + "'";
            com.CommandType=CommandType.Text;
            ds = db.Consulta(com);
            return GetByKey(ds);
        //}
    ...
    public Int32 CurrentPosicion(int? KeyClave)
    {
        ...
        {
            if (Function.FormatNumberNull(dt.Rows[i]["Rol_de_Usuario_Clave"]) == KeyClave)
                return i;
        }
        return -1;
    }
