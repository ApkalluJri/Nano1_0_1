...
public class objectBussinessSeccion : IDisposable
{
    ...
    private SeccionFilters[] filters; 
    ...
    public SeccionFilters[] Filters
    {
        get { return filters; }
        set { filters = value; }
    }
    public SeccionFilters Filter
    {
        ///Cuando es llamada desde las relaciones de algun catalago
        set
        {
            filters = new SeccionFilters[1];
            filters[0] = value;
        }
    }
    ...
    public Int32 SelCount()
    {
        ...
        if (swhere != "")
        {
            ...
            com.CommandText = "sp_SelAllComplete_Seccion_WithFilter";
        }else 
            com.CommandText = "sp_SelAllComplete_Seccion";
            ...
            if (this.filters !=null)
                foreach (SeccionCS.SeccionFilters fil in this.filters)
                {
                    String sWhereT = "";
                    sWhereT = Function.CreateQuerySelect("Seccion", fil);
                    if (sWhereT != "")
    ...
    public DataSet SelAll(Boolean ConRelaciones)
    {
        ...
        if (ConRelaciones ==true)
            ...
            com.CommandText="sp_SelAllSeccion";
            ds = db.Consulta(com);
            ...
            if (this.filters !=null)
                foreach (SeccionCS.SeccionFilters fil in this.filters)
                {
                    ...
                    if (sWhereT != "")
                        swhere = swhere + " And "+sWhereT;
                    String sOrderByT = "";
                    sOrderByT = Function.CreateOrderBySelect("Seccion", fil);
                    if (sOrderByT != "")
                        sOrderBy = sOrderByT;
                }
        if (swhere != "")
    ...
    public int SelCount(string sortDirection, string sortExpression, int maximumRows, int startRowIndex)
    {
        ...
        string sPagingCountTemplate = @"SELECT COUNT(*) {0}";
                  string from = " from  Seccion WITH(NoLock)  " + swhere; 
    ...
    public DataSet SelAll(string sortDirection, string sortExpression, int maximumRows, int startRowIndex)
    {
        ...
        if (swhere != string.Empty)
            swhere = (!swhere.ToLower().Trim().StartsWith("where") ? " WHERE " + swhere : swhere);
                    ...
                    switch (sortExpression)             {                 case "Seccion_Clave": sortExpression = "Seccion.Clave"; break;  case "Seccion_Descripcion": sortExpression = "Seccion.Descripcion"; break;                               }              sortExpression = (sortExpression == string.Empty || sortExpression == null ? "Seccion.Clave, Seccion.Descripcion" : sortExpression);             if (sortDirection != string.Empty)                 sortDirection = (sortDirection.ToLower() == "ascending" ? "Asc" : "Desc");             string fieldsWithAlias = " Seccion.Clave As Seccion_Clave,        Seccion.Descripcion As Seccion_Descripcion ";             string fieldsForExport = " Seccion.Clave As [Clave],        Seccion.Descripcion As [Descripcion] ";             string from = " from  Seccion WITH(NoLock)  " + swhere; 
    ...
    {
        	swhere = where; 	string fieldsWithAlias = " Seccion.Clave As Seccion_Clave,        Seccion.Descripcion As Seccion_Descripcion ";         Order = (Order == string.Empty || Order == null ? "Seccion.Clave, Seccion.Descripcion" : Order);         string from = " from  Seccion WITH(NoLock)  " + swhere;
    ...
    public void Insert(TTSecurity.GlobalData UserInformation, TTDataLayerCS.DataLayerFieldsBitacora DataReference){
        ...
        db.BeginTransaction("Seccion");
        int? sKeyClave = -1;
        try
        {

            SqlCommand com = new SqlCommand();
            com.CommandText="sp_InsSeccion";
        ...
        catch (Exception ex)
        {
            try
            {
                db.RollBackTransaction("Seccion");
            }
            catch{}
            throw ex; 
        }
...
com.Parameters.AddWithValue("@Clave",KeyClave);
    ...
    private DataSet GetByKey(DataSet ds)
    {
            ...
            {
            Clave = Conversion.CambiaToShort(ds.Tables[0].Rows[0]["Seccion_Clave"]);
            Descripcion = ds.Tables[0].Rows[0]["Seccion_Descripcion"].ToString();
...
//        {
            ...
            SqlCommand com = new SqlCommand();
            if (ConRelaciones==true)
                com.CommandText="sp_GetComplete_Seccion";
            else
                com.CommandText="sp_GetSeccion";
com.Parameters.AddWithValue("@Clave",KeyClave);
...
//        {
            TTDataLayerCS.BD db =new TTDataLayerCS.BD();
            SqlCommand com = new SqlCommand();
            com.CommandText="Select         Seccion.Clave As Seccion_Clave,        Seccion.Descripcion As Seccion_Descripcion   from  Seccion           Where Seccion.Clave = '" + Key + "'";
            com.CommandType=CommandType.Text;
            ds = db.Consulta(com);
            return GetByKey(ds);
        //}
...
//        {
            TTDataLayerCS.BD db =new TTDataLayerCS.BD();
            SqlCommand com = new SqlCommand();
            com.CommandText="Select         Seccion.Clave As Seccion_Clave,        Seccion.Descripcion As Seccion_Descripcion   from  Seccion           Where Seccion.Descripcion = '" + Key + "'";
            com.CommandType=CommandType.Text;
            ds = db.Consulta(com);
            return GetByKey(ds);
        //}
    ...
    public Int32 CurrentPosicion(int? KeyClave)
    {
        ...
        {
            if (Function.FormatNumberNull(dt.Rows[i]["Seccion_Clave"]) == KeyClave)
                return i;
        }
        return -1;
    }
